...
G4VPhysicalVolume* SLOWPOKEConstructor::ConstructWorld()
{
	...
	if(geomChanged)
	{
		...
		std::stringstream latticeType2;
		G4RotationMatrix* zRot = new G4RotationMatrix;
		G4ThreeVector latCellPos;
		G4ThreeVector holePos;
		G4ThreeVector firstHole((holePat[0]*cos(holePat[2])), (holePat[0]*sin(holePat[2])),0.);
		G4ThreeVector secondHole((holePat[0]*cos(holePat[2]-holePat[1])), (holePat[0]*sin(holePat[2]-holePat[1])),0.);
		G4ThreeVector bottomHole(0., -holePat[0], 0.);
		G4ThreeVector originHoleSec[6];
//		G4ThreeVector originTubeSec[6];
		G4int holeSecEndCoor[6][2] = {{12,13}, {11,5}, {10,5}, {8,12}, {7,10}, {6,7}};
		G4int tubeSecEndCoor[6][2] = {{12,13}, {11,11}, {10,9}, {9,10}, {7,12}, {6,10}};
		G4int originStartIndex[6] = {1,2,2,2,2,0};
		G4ThreeVector originStartPos[3] = {bottomHole, G4ThreeVector(0.,0.,0.), firstHole};
		originHoleSec[0]=G4ThreeVector(0., 0., 0.);
		originHoleSec[1]=G4ThreeVector(((12-11)*(-latticeCellDim[0]*0.5)+(4-12)*latticeCellDim[0]), ((12-11)*(latticeCellDim[1])), 0.);
		originHoleSec[2]=G4ThreeVector(((12-10)*(-latticeCellDim[0]*0.5)+(4-12)*latticeCellDim[0]), ((12-10)*(latticeCellDim[1])), 0.);
		originHoleSec[3]=G4ThreeVector(((12-8)*(-latticeCellDim[0]*0.5)+(11-12)*latticeCellDim[0]), ((12-8)*(latticeCellDim[1])), 0.);
		originHoleSec[4]=G4ThreeVector(((12-7)*(-latticeCellDim[0]*0.5)+(9-12)*latticeCellDim[0]), ((12-7)*(latticeCellDim[1])), 0.);
		originHoleSec[5]=G4ThreeVector(((12-5)*(-latticeCellDim[0]*0.5)+(12-12)*latticeCellDim[0]), ((12-5)*(latticeCellDim[1])), 0.);

		originTubeSec[0]=G4ThreeVector(((10-12)*latticeCellDim[0]), 0., 0.);
		originTubeSec[1]=G4ThreeVector(((12-11)*(-latticeCellDim[0]*0.5)+(9-12)*latticeCellDim[0]), ((12-11)*(latticeCellDim[1])), 0.);
		originTubeSec[2]=G4ThreeVector(((12-10)*(-latticeCellDim[0]*0.5)+(7-12)*latticeCellDim[0]), ((12-10)*(latticeCellDim[1])), 0.);
		originTubeSec[3]=G4ThreeVector(((12-9)*(-latticeCellDim[0]*0.5)+(9-12)*latticeCellDim[0]), ((12-9)*(latticeCellDim[1])), 0.);
		originTubeSec[4]=G4ThreeVector(((12-7)*(-latticeCellDim[0]*0.5)+(11-12)*latticeCellDim[0]), ((12-7)*(latticeCellDim[1])), 0.);
		originTubeSec[5]=G4ThreeVector(((12-6)*(-latticeCellDim[0]*0.5)+(8-12)*latticeCellDim[0]), ((12-6)*(latticeCellDim[1])), 0.);
...
//        new G4Box("testBox",  2.5*cm, 2.5*cm, 2.5*cm);
//
        ...
        //create the mothervolume for a 1/3 slice of the fuel bundle (contains a reflected pair of 1/6 slices of the fuel bundle)
        new G4Tubs("coreWaterSlice",  contRodZirTubeDim[1], refAnnDim[0], sheatheDim[2]/2, 2*CLHEP::pi/3, 2*CLHEP::pi/3);

        //creates the base zirconium grid slice that the holes will be subtracted from to form the upper and lower grid plate
        new G4Tubs("gridPlate",  gridPlateDim[0], gridPlateDim[1], gridPlateDim[2]/2, 2*CLHEP::pi/3, CLHEP::pi/3);

		// creates the three types of holes to be used to create the two hole patterns, the upGridHolesLat and lowGridHolesLat
		new G4Tubs("upGridHole1", upperGridHolesDim[0], upperGridHolesDim[1], upperGridHolesDim[2]/2, 0., 2.0*CLHEP::pi);
		new G4Tubs("lowGridHole1", lowGridHolesDim[0], lowGridHolesDim[1], lowGridHolesDim[2]/2, 0., 2.0*CLHEP::pi);
		...
		new G4Tubs("sheatheTubeH", sheatheDim[0], sheatheDim[1], sheatheDim[2]/2, 0., CLHEP::pi);
		new G4Tubs("sheatheTubeHR", sheatheDim[0], sheatheDim[1], sheatheDim[2]/2, 2*CLHEP::pi/3, CLHEP::pi);

        // creates the 6 base patterns of holes for the upGridHolesLat and lowGridHolesLat
		for (G4int i=1; i<6; i++)
		{
            latticeType1.str("");
            latticeType2.str("");
            ...
            holePos.set((holePat[0]*cos(holePat[1]*(-i)+holePat[2])), (holePat[0]*sin(holePat[1]*(-i)+holePat[2])),0.);
            new G4UnionSolid("upGridHole"+latticeType2.str(), theSolids->GetSolid("upGridHole"+latticeType1.str()),theSolids->GetSolid("upGridHole1"), 0, holePos-firstHole);
            new G4UnionSolid("lowGridHole"+latticeType2.str(), theSolids->GetSolid("lowGridHole"+latticeType1.str()), theSolids->GetSolid("lowGridHole1"), 0, holePos-firstHole);
		}

        // creates 5 more lowGridHolesLat patterns by adding a centre hole to some of the base patterns
        new G4UnionSolid("lowGridHole1C", theSolids->GetSolid("lowGridHole1"), theSolids->GetSolid("lowGridCentreHole"), 0, -bottomHole);
        new G4UnionSolid("lowGridHole2C", theSolids->GetSolid("lowGridHole2"), theSolids->GetSolid("lowGridCentreHole"), 0, -firstHole);
		new G4UnionSolid("lowGridHole3C", theSolids->GetSolid("lowGridHole3"), theSolids->GetSolid("lowGridCentreHole"), 0, -firstHole);
		new G4UnionSolid("lowGridHole4C", theSolids->GetSolid("lowGridHole4"), theSolids->GetSolid("lowGridCentreHole"), 0, -firstHole);
		new G4UnionSolid("lowGridHole6C", theSolids->GetSolid("lowGridHole6"), theSolids->GetSolid("lowGridCentreHole"), 0, -firstHole);
...
//		new G4Tubs("fuelPinH", fuelPinDim[0], fuelPinDim[1], fuelPinDim[2]/2, 0., CLHEP::pi);
        ...
        //creates the six sections of the upGridHolesLat and lowGridHolesLat (the union of these sections create the upGridHolesLat and lowGridHolesLat)
        //each of these sections contains 10 hole pattens (except for section0 which only has 9)

        //defines two arrays that contain the six sections of the upGridHolesLat and lowGridHolesLat
        G4UnionSolid* upGridHolesSec[6];
		G4UnionSolid* lowGridHolesSec[6];

        /*
        creates the intial unionsolid for each section. The initial unionsolid of each section represents the last two hole patterns of that section
        and the origin of the last hole pattern is used as the origin of the section.
        */

		latCellPos.set(-latticeCellDim[0]*0.5, bottomHole[1]+latticeCellDim[1], 0.);
		upGridHolesSec[0] = new G4UnionSolid("upGridHolesSec0", theSolids->GetSolid("upGridHole1"), theSolids->GetSolid("upGridHole1"), 0, latCellPos);
		lowGridHolesSec[0] = new G4UnionSolid("lowGridHolesSec0", theSolids->GetSolid("lowGridHole1"), theSolids->GetSolid("lowGridHole1C"), 0, latCellPos);

        latCellPos.set(-latticeCellDim[0], 0., 0.);
		upGridHolesSec[1] = new G4UnionSolid("upGridHolesSec1", theSolids->GetSolid("upGridHole3"), theSolids->GetSolid("upGridHole6"), 0, latCellPos);
		lowGridHolesSec[1] = new G4UnionSolid("lowGridHolesSec1", theSolids->GetSolid("lowGridHole3"), theSolids->GetSolid("lowGridHole6"), 0, latCellPos);

		upGridHolesSec[3] = new G4UnionSolid("upGridHolesSec3", theSolids->GetSolid("upGridHole3"), theSolids->GetSolid("upGridHole3"), 0, latCellPos);
		lowGridHolesSec[3] = new G4UnionSolid("lowGridHolesSec3", theSolids->GetSolid("lowGridHole3C"), theSolids->GetSolid("lowGridHole3C"), 0, latCellPos);

		upGridHolesSec[4] = new G4UnionSolid("upGridHolesSec4", theSolids->GetSolid("upGridHole3"), theSolids->GetSolid("upGridHole3"), 0, latCellPos);
		lowGridHolesSec[4] = new G4UnionSolid("lowGridHolesSec4", theSolids->GetSolid("lowGridHole3C"), theSolids->GetSolid("lowGridHole3"), 0, latCellPos);

		upGridHolesSec[5] = new G4UnionSolid("upGridHolesSec5", theSolids->GetSolid("upGridHole1"), theSolids->GetSolid("upGridHole3"), 0, latCellPos+firstHole-bottomHole);
		lowGridHolesSec[5] = new G4UnionSolid("lowGridHolesSec5", theSolids->GetSolid("lowGridHole1C"), theSolids->GetSolid("lowGridHole3C"), 0, latCellPos+firstHole-bottomHole);

        latCellPos.set((12-9)*(-latticeCellDim[0]*0.5), (12-9)*(latticeCellDim[1]), 0.);
		upGridHolesSec[2] = new G4UnionSolid("upGridHolesSec2", theSolids->GetSolid("upGridHole6"), theSolids->GetSolid("upGridHole1"), 0, bottomHole+latCellPos-firstHole-originHoleSec[2]);
		lowGridHolesSec[2] = new G4UnionSolid("lowGridHolesSec2", theSolids->GetSolid("lowGridHole6"), theSolids->GetSolid("lowGridHole1"), 0, bottomHole+latCellPos-firstHole-originHoleSec[2]);

        /*
         this section of the code essentially: reads through the top left quarter of the latticeMat data matrix,
         checks whether the current section (indexed by 'k') of the upGridHolesLat and lowGridHolesLat has been filled
         (by checking the count of how many solids have been added to 'g') and if so moves to the next section
         (by decrementing 'k' and setting the read position 'i' and j to the start of the next section)
         determines the needed hole pattern from the read value,
         calculates the the position of the hole pattern relative to the current section
         and adds the selected hole pattern to the current section
         */
            ...
            while (j<13)
            {
                if(latticeMat[i][j]!=7)
                {
                    if(index==9||(index==8&&k==0))
                    {
                        ...
                        beside=(latticeMat[i][j-1]!=7) ? true:false;
                    ...
                    if(beside)
                    {
                        if(j!=12)
                        {
                            ...
                            latCellPos=latCellPos+firstHole-originStartPos[originStartIndex[k]];
                        }

                        else
                        {
                            latticeType2 << 1;
                            latCellPos=latCellPos+bottomHole-originStartPos[originStartIndex[k]];
                        }
                    }
                    else if((latticeMat[i-1][j+1] != 7)&&(latticeMat[i-1][j] == 7))
                    {
                        latticeType2 << 4;
                        latCellPos=latCellPos+secondHole-originStartPos[originStartIndex[k]];
                        rot=true;
                    }
                    ...
                    if(!rot)
                    {
                        ...
                        upGridHolesSec[k] = new G4UnionSolid("upGridHolesSec"+latticeType1.str(), upGridHolesSec[k], theSolids->GetSolid("upGridHole"+latticeType2.str()),
                                                            0, latCellPos-originHoleSec[k]);

                        if(latticeMat[i][j]==9)
                           latticeType2 << "C";

                        lowGridHolesSec[k] = new G4UnionSolid("lowGridHolesSec"+latticeType1.str(), lowGridHolesSec[k], theSolids->GetSolid("lowGridHole"+latticeType2.str()),
                                                            0, latCellPos-originHoleSec[k]);
                    }
                    else
                    {
                        ...
                        upGridHolesSec[k] = new G4UnionSolid("upGridHolesSec"+latticeType1.str(), upGridHolesSec[k], theSolids->GetSolid("upGridHole"+latticeType2.str()),
                                                            G4Transform3D(*zRot, latCellPos-originHoleSec[k]));
		...
		//creates the six sections of the sheatheLat
        //each of these sections contains 6 hole pattens (except for section0 which only has 7)

        //defines two arrays that contain the six sections of the sheatheLat
        G4UnionSolid* sheatheLatSec[6];

        /*
        creates the intial unionsolid for each section. The initial unionsolid of each section represents the last two sheathe tubes of that section
        and the origin of the last sheathe tube is used as the origin of the section.
        */

		latCellPos.set(-latticeCellDim[0]*2, 0., 0.);
		sheatheLatSec[1] = new G4UnionSolid("sheatheLatSec0", theSolids->GetSolid("sheatheTube"), theSolids->GetSolid("sheatheTube"), 0, latCellPos);

        latCellPos.set(-latticeCellDim[0], 0., 0.);
		sheatheLatSec[0] = new G4UnionSolid("sheatheLatSec0", theSolids->GetSolid("sheatheTubeH"), theSolids->GetSolid("sheatheTubeH"), 0, latCellPos);

		sheatheLatSec[2] = new G4UnionSolid("sheatheLatSec2", theSolids->GetSolid("sheatheTube"), theSolids->GetSolid("sheatheTube"), 0, latCellPos);

		sheatheLatSec[4] = new G4UnionSolid("sheatheLatSec4", theSolids->GetSolid("sheatheTube"), theSolids->GetSolid("sheatheTube"), 0, latCellPos);

		sheatheLatSec[5] = new G4UnionSolid("sheatheLatSec5", theSolids->GetSolid("sheatheTube"), theSolids->GetSolid("sheatheTube"), 0, latCellPos);

        latCellPos.set(((12-8)*(-latticeCellDim[0]*0.5)+(8-12)*latticeCellDim[0]), (12-8)*(latticeCellDim[1]), 0.);
		sheatheLatSec[3] = new G4UnionSolid("sheatheLatSec3", theSolids->GetSolid("sheatheTube"), theSolids->GetSolid("sheatheTube"), 0, latCellPos-originTubeSec[3]);
		...
		while (i<13)
		{
            while (j<13)
            {
                if(latticeMat[i][j]==8)
                {
                    ...
                    if((j!=12)&&(i!=12))
                    {
                        sheatheLatSec[k] = new G4UnionSolid("sheatheLatSec"+latticeType1.str(), sheatheLatSec[k], theSolids->GetSolid("sheatheTube"),
                                                            0, latCellPos-originTubeSec[k]);
                    }
                    else if(j==12)
                    {
                        sheatheLatSec[k] = new G4UnionSolid("sheatheLatSec"+latticeType1.str(), sheatheLatSec[k], theSolids->GetSolid("sheatheTubeHR"),
                                                            0, latCellPos-originTubeSec[k]);
                    }
                    else
                    {
                        sheatheLatSec[k] = new G4UnionSolid("sheatheLatSec"+latticeType1.str(), sheatheLatSec[k], theSolids->GetSolid("sheatheTubeH"),
                                                            0, latCellPos-originTubeSec[k]);
                    }
...
//		delete zRot;
		...
		// adds the four missing center holes to the bottom of the lowGridHolesSec0 (the bottom of the lowGridHolesLat)

		latCellPos.set(-latticeCellDim[0]*4, 0., 0.);
		lowGridHolesSec[0] = new G4UnionSolid("lowGridHolesSec0", lowGridHolesSec[0], theSolids->GetSolid("lowGridCentreHole"), 0, latCellPos);

		latCellPos.set(-latticeCellDim[0]*6, 0., 0.);
		lowGridHolesSec[0] = new G4UnionSolid("lowGridHolesSec0", lowGridHolesSec[0], theSolids->GetSolid("lowGridCentreHole"), 0, latCellPos);

		latCellPos.set(-latticeCellDim[0]*7, 0., 0.);
		lowGridHolesSec[0] = new G4UnionSolid("lowGridHolesSec0", lowGridHolesSec[0], theSolids->GetSolid("lowGridCentreHole"), 0, latCellPos);

        // creates the upGridHolesLat and lowGridHolesLat from their six sections

		G4UnionSolid* upGridHolesLat = new G4UnionSolid("upGridHolesLat", upGridHolesSec[0], upGridHolesSec[5], 0, bottomHole+originHoleSec[5]);
		...
		for(i=1; i<5; i++)
		{
            ...
            sheatheLat = new G4UnionSolid("sheatheLat", sheatheLat, sheatheLatSec[i], 0, originTubeSec[i]-originTubeSec[0]);
		}
		...
		new G4SubtractionSolid("upGridPlate", theSolids->GetSolid("gridPlate"), upGridHolesLat, 0, originHoleSec[0]);
		new G4SubtractionSolid("lowGridPlate", theSolids->GetSolid("gridPlate"), lowGridHolesLat, 0, originHoleSec[0]);
		...
		new G4UnionSolid("zircGridSlice", theSolids->GetSolid("zircGridSliceP1"), sheatheLat, 0, disUpGridToSheathe+originTubeSec[0]);
...
//        //creates the full ziconium grid by adding up the slices
		new G4UnionSolid("zircGridPlate1/3", theSolids->GetSolid("zircGridSlice"), theSolids->GetSolid("zircGridSliceRefl"), 0, originHoleSec[0]);
//
...
//		new G4UnionSolid("zircGridPlate", theSolids->GetSolid("zircGridPlate2/3"), theSolids->GetSolid("zircGridPlate1/3"), G4Transform3D(*zRot, originHoleSec[0]));
    ...
    zircGridLogical = new G4LogicalVolume(theSolids->GetSolid("zircGridPlate1/3"), matMap["Zirconium"], "zircGridLogical");

    new G4PVPlacement(0, -disUpGridToSheathe, zircGridLogical,"zircGridSlicePhysical",
