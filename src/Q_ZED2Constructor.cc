...
// ConstructNewWorld()
G4VPhysicalVolume*
Q_ZED2Constructor::ConstructNewWorld(const StorkParseInput* infile)
{
...
G4VPhysicalVolume* Q_ZED2Constructor::ConstructWorld()
{



	// Set local variables and enclosed world dimensions
	reactorDim = G4ThreeVector(0.*cm,  231.806*cm/2. ,405.4*cm/2.);
	G4double buffer = 1.0*cm;
	encWorldDim = G4ThreeVector(2.*reactorDim[1]+buffer, 2.*reactorDim[1]+buffer, 2*reactorDim[2]+buffer);
 ...
 G4double XPos[] = {
  ...
  G4double YPos[] = {
    ...
    // Create the end plate
        ...
        new G4PVPlacement(0,G4ThreeVector(-reactorDim[1],-reactorDim[1],Graphitebott[2]), vesselLogical, "VesselPhysical",worldLogical , 0, 0);
       ...
       // Create  lower coolant in mother air volume
        logicCoolant1 = new G4LogicalVolume(theSolids->GetSolid("CoolantTube1Cut2"), matMap["Air"], "Coolant1Logical", 0, 0, 0);
       ...
       // Create  lower coolant in moderator volume
        logicCoolant1Mod = new G4LogicalVolume(theSolids->GetSolid("CoolantTube1Cut1"), matMap["Air"], "Coolant1Logical", 0, 0, 0);
...
// ConstructMaterials()
// Construct all the materials needed for the Q_ZED2Constructor.
void Q_ZED2Constructor::ConstructMaterials()
{
