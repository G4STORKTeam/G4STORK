...
#include <fstream>
  ...
  const G4String StorkNeutronHPNames::theString[100] = {"Hydrogen", "Helium",
 ...
 "Einsteinium","Fermium"};
  ...
  G4String StorkNeutronHPNames::GetName(G4int i) { return theString[i]; }

//G4NeutronHPDataUsed StorkNeutronHPNames::GetName(G4int A, G4int Z, G4String base, G4String rest, G4bool & aFlag)
G4NeutronHPDataUsed StorkNeutronHPNames::GetName(G4int A, G4int Z, G4int M, G4String base, G4String rest, G4bool & aFlag)
{
...
if(getenv("NeutronHPNames")) G4cout << "Names::GetName entered for Z = " << Z << ", A = " << A <<G4endl;
    ...
    if(Z>92.5&&!getenv("AllowForHeavyElements") )
    {
      ...
      throw G4HadronicException(__FILE__, __LINE__, "StorkNeutronHPNames::GetName - data with Z>92 are not provided");
    }
...
//        if (std::abs(myZ-Z)>theMaxOffSet||myZ==0||myA==0)
          if ( delta_Z > theMaxOffSet )
          {
             ...
             //else
             //{
                G4cout <<"StorkNeutronHPNames: Sorry, this material does not come near to any data."<<G4endl;
                G4cout <<"StorkNeutronHPNames: Please make sure G4NEUTRONHPDATA points to the" << G4endl;
                G4cout <<"                  directory, the neutron scattering data are located in." << G4endl;
                G4cout << "StorkNeutronHPNames: The material was: A="<<A<<", Z="<<Z<<G4endl;
